...
namespace HealthCare.Infreastructure.Services
{
    public class AlertService : IAlertService
    {
        ...
        public async Task GenerateAlertsFromMeasurementAsync(SensorMeasurement m, Guid userId)
        {
            var alerts = new List<Alert>();

            if (m.bpm > 100)
                alerts.Add(MakeAlert(userId, AlertType.Critical, AlertCategory.HeartRate,
                    "High Heart Rate Detected",
                    $"Your heart rate reached {m.bpm} BPM. Please consult a doctor if this persists."));
            ...
            if (m.spo2 < 95)
                alerts.Add(MakeAlert(userId, AlertType.Warning, AlertCategory.SpO2,
                    "Low SpO2 Level",
                    $"Blood oxygen dropped to {m.spo2}%. Take deep breaths and monitor."));

            if (m.temp > 38.5)
                alerts.Add(MakeAlert(userId, AlertType.Warning, AlertCategory.Temperature,
                    "High Temperature Detected",
                    $"Your temperature is {m.temp}°C. You may have a fever."));

            if (m.fall_detected)
                alerts.Add(MakeAlert(userId, AlertType.Critical, AlertCategory.FallDetected,
                    "Fall Detected!",
                    $"A fall has been detected ({m.fall_type}). Are you okay?"));

            if (!alerts.Any())
                alerts.Add(MakeAlert(userId, AlertType.Info, AlertCategory.HeartRate,
                    "All Good ✅",
